...
app.run(function() {
    ...
    var MAX_DELTA_COUNT = 12;
    // var BEAM_WIDTH = 25; // only look at top BEAM results
    var MIN_RELATIVE_THRESHOLD = -3.9    ; // prev to top scoree
    // var QUEUE_THRESHOLD = 5;
    // var MAX_QUEUE_THRESHOLD = 6; // always limit queue items within MAX and MIN QUEUE THRESHOLD
    // var MIN_QUEUE_THRESHOLD = 3; // always queue items within MIN_QUEUE_THRESHOLD of current score
    // var MIN_SCORE = 40;6
    var MAX_COUNT = 3000;
    var TEXT = me.t1000;
    var SWAPPABLE_KEYS = me.nSemi13Mask;
    ...
    var firstKeyboard = _.cloneDeep(KB.keySet.standard.nsemi);
    firstKeyboard.layoutName = "ROOT KEYBOARD LAYOUT NAME";
    console.log('Keyboard', me.toKeyString(firstKeyboard));
    ...
    var scored = {}
    var completed = {}
    q.push({keyString: me.toKeyString(firstKeyboard), layoutName: firstKeyboard.ops, score: initialScore, /*scoreList: [],*/ keySet: firstKeyboard}); 
    ...
    while(q.length > 0 && count <= MAX_COUNT) {
        ...
        var flatLayoutName = sortAndFlattenString(current.layoutName);
        
        console.log({"pop!": currentLayoutName, score: current.score, delta: flatLayoutName, step: "" + count + " / " + MAX_COUNT + " (" + q.length + ")", best: bestLayoutName + " " + bestScore + " " + bestString});
        ...
        _.forEach(swappableCombinations, function(keyIxList, pairCount){  
            ...
            var flatLayoutName = sortAndFlattenString(nextLayoutName);
            nextKeySet.layoutName = nextLayoutName
            me.rotateKey(nextKeySet, keyIxList);
            
            if (nextLayoutName in completed) {
                // console.log("bail exists in completed");
                return;
            } else if (flatLayoutName.length > MAX_DELTA_COUNT ) {
                // console.log("bail too long");
                return;
            } 
        ...
        _.forEach(scores.finalList, function(final, finalCount){
            ...
            var finalDeltaScore = final.score - current.score; // positive means better, negative worse
            var finalRelativeScore = final.score - topfinalScore; // negative is worse (usually between 0 and -3.5)
            // finalScoreList.push({deltaScore: final.score - current.score, relativeScore: finalRelativeScore, delta: deltaName});

            completed[finalName] = {layoutName: finalName, delta: deltaName, keyString: finalKeyString, score: final.score};
            ...
            // Only queue/recurse if above minimum score and score doesn't exist
            // var queueThreshold = 10;
            // var queueRatio = (1 - deltaName.length / MAX_DELTA_COUNT);
            // var queueThreshold = (MAX_QUEUE_THRESHOLD - MIN_QUEUE_THRESHOLD) * queueRatio + MIN_QUEUE_THRESHOLD; 
            // var queueThreshold = MAX_DELTA_COUNT - deltaName.length + MIN_QUEUE_THRESHOLD; // Math.max(MIN_QUEUE_THRESHOLD, );

            if (final.score.toString() in scored) {
                // console.log("no queue: scored found");
                return;
            }
            ...
            // if (final.score <= bestScore - QUEUE_THRESHOLD) {
            //     console.log("no queue: BELOW threshold");
            //     return;
            // }z

            // if (topfinalScore - final.score < 5) {
            //     console.log("no queue: TOP too far");
            //     return;
            // }

            // if (finalCount > BEAM_WIDTH) {
            //     console.log("no queue: not in beam");
            //     return;
            // }
            console.log({save: final.score, /*scoreList: finalScoreList,*/ delta: deltaName, ops: finalName + " | " + finalKeyString});

            var result = {score: final.score, keyString: finalKeyString, score: final.score, /*scoreList: finalScoreList,*/ delta: deltaName, layoutName: finalName, keySet: final.keySet};
            scored[final.score.toString()] = result;

            if (finalDeltaRemaining <= 0) {
                // console.log("no queue: done");
                return;
            }

            console.log({push: finalName, score: final.score, /*scoreList: finalScoreList,*/ delta: "" + deltaName + "  " + finalKeyString});
            q.push(result);
    
        });

    }
    console.log({bestScore: bestScore, bestString: bestString, bestName: bestLayoutName, qLen: q.length});
    var filteredScored = _.filter(Object.values(scored), function(o){ return o.score > bestScore - 5});
    var sortedScored = _.take(_.sortBy(filteredScored, "score").reverse(), 900);
